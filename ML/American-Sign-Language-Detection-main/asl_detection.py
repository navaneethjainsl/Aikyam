...
#     plt.title("Indian Sign Language Detector")
#     plt.axis('off')
#     plt.show()
...
with mp_hands.Hands(
    ...
    while cap.isOpened():
        ...
        # Check if hands are detected before proceeding
        if results.multi_hand_landmarks:
            # Draw the hand annotations on the image.
            image.flags.writeable = True
            image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks, results.multi_handedness):
                # Calculate landmarks and preprocess
                landmark_list = calc_landmark_list(debug_image, hand_landmarks)
                pre_processed_landmark_list = pre_process_landmark(landmark_list)
                ...
                # Convert landmarks into a dataframe for prediction
                df = pd.DataFrame(pre_processed_landmark_list).transpose()
            ...
            # Display the image with hand landmarks and label
            # plt.imshow(cv2.cvtColor(image, cv2.COLOR_BGR2RGB))
            # plt.title("Indian Sign Language Detector")
            # plt.axis('off')
            # plt.show()
        ...
        # else:
        #     print("No hand detected. Waiting for hand signs...")
