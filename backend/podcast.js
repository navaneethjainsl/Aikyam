...
async function taddyGraphqlRequest({ query, variables }) {
  ...
  try {
    ...
    if (data.searchForTerm) {
      console.log('Podcast Series:');
      data.searchForTerm.podcastSeries.forEach((series) => {
        console.log(series);  // This will log each series object
      });

      console.log('Podcast Episodes:');
      data.searchForTerm.podcastEpisodes.forEach((episode) => {
        console.log(episode);  // This will log each episode object
      });
    }
...
export { taddyGraphqlRequest, taddyQuery };